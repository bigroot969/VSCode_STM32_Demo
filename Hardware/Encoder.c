...
Encoder_ButtonState Encoder_GetButton(void)
{
    if (Encoder_Check(ENCODER_BTN_SINGLE))
        return ENCODER_BTN_SINGLE_ENUM;
    if (Encoder_Check(ENCODER_BTN_LONG))
        return ENCODER_BTN_LONG_ENUM;
...
{
    // --- 编码器旋转检测 (双边沿检测) ---
    // 优化：一次性读取端口状态，确保信号同步，并减少函数调用开销
    uint16_t portData = GPIOB->IDR;
    uint8_t A = (portData & GPIO_Pin_6) ? 1 : 0;
    uint8_t B = (portData & GPIO_Pin_7) ? 1 : 0;
    ...
    // --- 按键检测 (状态机) ---
    uint8_t key_phys = (portData & GPIO_Pin_8) ? 1 : 0; // 0=按下, 1=松开
